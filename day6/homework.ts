...
*/
function handleResponse(status_code :number,response:string){
...
const missingnumber = function (arr:any):any{
for (let i=0;i<arr.length-1;i++){
if(arr[i]!== i+1){
   return i+1;
}
...
function hasPermission(role: Roles, permission: string): boolean {
   //const permissions = rolePermissions[role];
   return rolePermissions[role][permission] === true;
   
}

console.log(`Admin has delete permission: ${hasPermission(Roles.Admin, 'delete')}`); 
console.log(`Editor has create permission: ${hasPermission(Roles.Editor, 'create')}`); 
console.log(`Viewer has edit permission: ${hasPermission(Roles.Viewer, 'edit')}`);
